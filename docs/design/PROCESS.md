...
**기초공사 (INGEST Foundation)** — 계약을 조금만 어겨도 분석이 그럴듯하게 깨지므로, **LLM 전 읽기 게이트** + **분기마다 pytest**:

```
Branch-0 (지금)   Phase R  μ-R-*  →  pytest test_branch_gates + phase_r_regression  (LLM 0)
Branch-1 (quota)  Phase A  S0-B/C →  BRANCH-1-spec.md + manual E2E
Branch-2a/2b/3    잠금 · 착수 금지 (quota 후 Branch-1만)
```
...
| Closure | S0-A/B/C E2E | `STAGE-0-CLOSURE-ROADMAP.md` · **`BRANCH-1-spec.md`** (quota 후) |
