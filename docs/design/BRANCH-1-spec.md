...
## REG-B1-smoke — pipeline_batch S5 touch 후 짧은 회귀

`pipeline_batch.py` μ-PROBE-S5 (`LINK_DISABLE_NEO4J_AUTO_START`) 반영 후, full REG-B1b(~542s) 전 **L0~L1** 확인.

| μ-ID | 내용 | 검증 |
|------|------|------|
| **μ-REG-S0** | ω-0 baseline | `stage0_reaudit_baseline.py` + `phase_r_regression.py` exit 0 |
| **μ-REG-L1-py** | ingest touch + S5 contract | `test_pipeline_neo4j_probe_skip.py`, `test_ingest_foundation.py` |
| **μ-REG-L1-live** | S0-B short Phase A (batch) | `scripts/branch1_regression_smoke.py` → `logs/branch1_regression/*-smoke.json` |
| **μ-REG-B1-smoke-ω** | 주석·sample | 본 §, 스크립트 헤더, `branch1_regression_smoke_sample.json` |

**금지:** `branch1_full_e2e.py` (full REG-B1b). **합격:** smoke `pipeline_ok=true` (관측 숫자는 참고).

### 관측 (2026-06-23, post S5)
...
| log | `logs/branch1_regression/20260623-1002-smoke.json` |
