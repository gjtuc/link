...
> **상태:** μ-2b-02-API + **μ-2b-02-UI** ✅  
> **선행:** μ-2b-01 (`ingest_hook`)  
> **다음:** **μ-2b-03** (실 bolt 영속 통합) — [STAGE-1-PERSIST-spec.md](STAGE-1-PERSIST-spec.md)
...
| **μ-2b-02-UI** | index.html corpus 한 줄 힌트 | `tests/test_stage1_corpus_ui.py` |
| **μ-2b-02-UI-FIX** | 분석 후 corpus 줄 갱신 + 사용자 문구 | `tests/test_stage1_corpus_ui.py` |
...
## UI 힌트 (μ-2b-02-UI / μ-2b-02-UI-FIX ✅)

**파일:** `web/index.html` — `GET /api/status` → `cross_run_corpus` 읽기 전용 한 줄.

| `enabled` | `run_count` | 표시 (사용자 문구) |
|-----------|-------------|-------------------|
| `false` | — | 숨김 (`show: false`) |
| `true` | `0` | `분석 기록 모음: 켜짐 · 아직 분석한 적 없음` |
| `true` | `>0` | `분석 기록 모음: N번 분석 · 항목 M개 · 출처 파일 K개` |

**JS:** `formatCrossRunCorpusLine` → `updateCrossRunCorpusHint` — `updateBackendStatus` 끝에서 호출.  
**갱신 (μ-2b-02-UI-FIX):** `refreshBackendStatusFromApi()` — 페이지 로드 + **분석 성공 후** `/api/status` 재조회로 corpus 줄 반영.  
**톤:** Fact-Checker 힌트 `#fc-hint` 주황(`#e37400`), 분석 기록 모음 `#corpus-hint` 회색(`#5f6368`).  
**DOM:** `#corpus-hint` (fc-hint 아래). 편집·env 토글 없음.

```bash
python -m pytest tests/test_stage1_corpus_ui.py -q
```

---

## NON-GOALS (μ-2b-00~02-UI)
