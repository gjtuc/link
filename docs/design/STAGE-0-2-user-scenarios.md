...
각 시나리오는 0-1의 γ(원인과 결과), ζ(요리 비유), C-*, NG-* 에 **매핑**된다.
...
| ID | 이름 | 0-1 근거 | 우선순위 |
|----|------|----------|----------|
| **S0-A** | 논문 PDF 1편 — 뼈대·원인→결과 지도 | α-2, α-4, ζ-1~4, β-1 | P0 |
| **S0-B** | 보고서 초안(텍스트) — 비약·약한 원인 | α-4, γ Gap/Weak, ζ-3 | P0 |
| **S0-C** | 자료 여러 편 — 뜻밖의 원인→결과 | α-4, **2단계** | P1 |
...
> **완성품(ζ-1)** 이 **청크 단위(β-1)** 로 해체되고, 원인과 결과 그래프와 (향후) 뼈대 강약을 본다.  
...
| B-2-3 | Skeptic 완료 | 약한 원인→결과는 기각 또는 weak (ζ-3) |
...
## S0-C — 자료 여러 편 · 교차 원인→결과 (P1)

### C-1. 사용자 스토리

> 논문 PDF + 보조 PDF(또는 메모 txt)를 **동시에** 넣는다.  
> **서로 다른 완성품** 사이에서 **뜻밖의 원인→결과**(브릿지)를 보고 싶다 (α-4, 2단계).
...
> 사용자 원인→결과 가설이 **Fact-Checker·Skeptic** 을 거친다.
